context("Equivalence between estimation methods with long and wide data")

test_that("Data structure (wide vs long) does not affect results", {
  ...
  expect_equal(dr_imp.did_panel$se, dr_imp.did_panel2$se)
  expect_equal(dr_trad.did_panel$se, dr_trad.did_panel2$se)
  expect_equal(std_ipw.did_panel$se, std_ipw.did_panel2$se)
  expect_equal(or.did_panel$se, or.did_panel2$se)
})

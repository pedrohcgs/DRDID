# Bootrstapped Intrinsically Efficient Doubly Robust Difference-in-Differences with Panel Data
# 2 periods and 2 groups

wboot.dr.ie.panel <- function(nn, n, deltaY, D, int.cov, i.weights){
  #-----------------------------------------------------------------------------
  v <- stats::rexp(n)
  #v <- v / mean(v)
  ...
  out.reg.b <- wols.ie.panel(deltaY, D, int.cov, ps.b, i.weights = b.weights)$out.reg
